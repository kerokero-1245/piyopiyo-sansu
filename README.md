...
src/
  ...
  settings.ts               かずの範囲 maxSum（5/10）・よみあげ on/off・累計⭐の保存（web は localStorage）
  game/problems.ts          問題生成（generateProblem / generateSet / 選択肢生成）
  audio/sounds.ts           効果音（web は Web Audio 合成、native は無音スタブ）
  audio/voice.ts            よみあげの入口。3段構え（①同梱クリップ→②speechSynthesis→③無音）に解決
  ...
  screens/
    ...
    OtonaScreen.tsx         おとなモード（5まで / 10まで の切替・よみあげ する/しない）
```
